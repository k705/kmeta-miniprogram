...
Page({
  ...
  data: {
    ...
    /** 签到*/
    isSignIn: true, //今日是否签到
    showsignIn: true,
    monthSignInDays: 0, //月签到天数
    rewardList: [{ //	奖励列表
      day: 0, //	天数
      important: 0, //重要奖励 0-不重要 1-重要
      rewardIcon: "", //	奖励图片
      rewardType: "", //	奖励类型
      rewardQuantity: 0 //奖励数量
    }],

    /** 签到道具信息*/
    showsignInfo: false,
    propName:'',
    rewardDesc: "", //	奖励说明	string	
    rewardIcon: "", //奖励icon(奖励为道具时，为svga动效)	string	
    rewardQuantity: "", //	奖励数量	integer(int32)	
    rewardType: "", //	奖励类型 score-K分 prop-道具	string
 ...
 demo(){
  ...
  wx.onDeviceMotionChange((result) => {
    ...
    var xVal = -(result.gamma).toFixed(2)/5;
      var yVal = -(result.beta+60).toFixed(2)/5;
      // console.log(yVal,'yVal');
      var zVal = -(result.alpha).toFixed(2)/5;
      self.setData({
        // x: xVal > 5 ? -35 : (xVal < -5 ? 35 : 1.5*xVal),
        // x1: yVal > 5 ? -35 : (yVal < -5 ? 35 : -1.5*yVal),
        // scale: xVal > 5 ? 1.1 : (xVal < -5 ? 1.1 : 1),
        y: yVal > 5 ? 5 : (yVal < -5 ? -5 : yVal),


        x: xVal > 5 ? -20 : (xVal < -5 ? 20 : 0),
        x1: yVal > 0.5 ? 20 : (yVal < 0.5 ? -20 : 0),

        //  x: zVal > 5 ? -25 : (zVal < -5 ? 35 : 0),
        // x1: zVal > 5 ? -55 : (zVal < -5 ? 55 : 0),
      })
  ...
  async signInV2() {
    ...
    console.log(rewardIcon,'rewardIcon');
    this.setData({
      propName,
      rewardDesc,
      rewardIcon,
  ...
  async getpropInfo() {
    ...
    var timer = setTimeout(() => {
      ...
      if (this.data.firstUseResidue !== undefined && this.data.firstUseResidue === 0 && (this.data.propType == 2 || this.data.propType == 4 || this.data.propType == 7) && useResidue !== 0) {
  ...
  async useProp(e) {
    ...
    try {
      ...
      if (this.data.propType == 2 || this.data.propType == 3 || this.data.propType == 4 || this.data.propType == 5|| this.data.propType == 6|| this.data.propType == 7|| this.data.propType == 8) {
        this.setData({
          useProp: true
  ...
  updateProgress: function () {
    ...
    if (this.data.propType == 6|| this.data.propType == 8) {
      firstUseResidue = this.data.firstUseResidue // 使用剩余时间
    }
    if (this.data.propType == 7) {
      firstUseResidue = 60; // 使用剩余时间
      console.log(firstUseResidue,'firstUseResidue');
    }
  ...
  async clickOre(event) {
    ...
    if (this.data.score > 500) {
      ...
      this.setData({
       ...
       // 1.5 秒后将 showBigScore 设置为 false
    setTimeout(() => {
      this.setData({
        showBigScore: false,
        BigScorecard_overlay:false
